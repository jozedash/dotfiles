...
EXTEND=2
GAMING=3
MODE_COUNT=5 # Used to ensure the loop resets
...
MONITOR2=$(hyprctl monitors all | grep "(ID 2)" | grep -oP "DP-\d")
if [ ! -z $MONITOR2 ]; then
    echo "Found third monitor on $MONITOR2"
fi
...
if [ -z $MONITOR ]; then
    ...
    if [ $STATE -ne $LAPTOP ]; then
        echo "Set to laptop output only!"
        STATE=$LAPTOP
    fi
else
